...
class Aperture(object):
    ...
    def Idl2Sci(self, XIdl, YIdl):
        ...
        XSci = self.XSciRef * np.ones_like(np.asarray(XIdl), dtype=float)
        YSci = self.YSciRef * np.ones_like(np.asarray(YIdl), dtype=float)

        for i in range(1,degree+1):
            for j in range(0,i+1):
                XSci += self.Idl2SciCoeffs_X[i,j] * np.power(XIdl, i-j) * np.power(YIdl, j)
                YSci += self.Idl2SciCoeffs_Y[i,j] * np.power(XIdl, i-j) * np.power(YIdl, j)
...
class SIAF(object):
    ...
    def __init__(self, instr='NIRISS', filename=None, basepath=None, **kwargs):
        ...
        if filename is None:
            if basepath is None:
                raise RuntimeError("Cannot determine filename (supply `filename` or `basepath` arguments)")
